...
int isFLoat(char *str, int strsize){
    ...
    for(i = 0; i < strsize; i++){
        if(!isdigit(str[i]) && (str[i] != '.') && (str[i] != '+') && (str[i] != '-') && ((int)str[i] != 13)) return 0;
        if(str[i] == '.') ctr1++;
        else
            if (str[i] == '+') ctr2++;
            else
    ...
    if(ctr1 > 1 || ctr2 > 1 || ctr3 >1) return 0;
    if(ctr1+ctr2 > 1) return 0; /* it checks if we have both '-' and '+' in the same float number */
    return 1;
}
...
int CheckHeader(char *line, int *nsamples, int *nlabels, int *nfeats){
    ...
    while (token){
        ...
        if(!isInteger(token, strlen(token)))
            return 0;
        switch (ctr){
            case 1: *nsamples = atoi(token);
                break;
            case 2: *nlabels = atoi(token);
                break;
            case 3: *nfeats = atoi(token);
                break;
            default: return 0;
        }
        token = strtok(NULL, " \n\t");
    ...
    if(ctr != 3) return 0;
    if((*nsamples <= 0) || (*nlabels <= 0) || (*nfeats <= 0))return 0;
...
int CheckLine(char *line, int nlabels, int nfeats, int lineid){
    ...
    token = strtok(line, " \n\t");
    if(!isInteger(token, strlen(token))) return 0;
    
    /* it checks the label of each sample (second column) */
    token = strtok(NULL, " \n\t");
    if(!isInteger(token, strlen(token)) || atoi(token) <= 0) return 0;
    
    /* it checks the features */
    token = strtok(NULL, " \n\t");
    while (token){
        if(!isFLoat(token, strlen(token))) return 0;
        ctr++;
        token = strtok(NULL, " \n\t");
    }
    
    if (ctr != nfeats) return 0;
